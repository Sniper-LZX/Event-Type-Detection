...
class DeepEmbeddingClustering(object):
    def __init__(self,
        ...
        self.encoders_dims = [self.input_dim] + encoders_dims
    ...
    def initialize(self, X, y=None, save_autoencoder=False, layerwise_pretrain_iters=50000, finetune_iters=100000):
        if self.pretrained_weights is None:
            ...
            if save_autoencoder:
                self.autoencoder.save_weights('autoencoder_weights_fnplus.h5')
        else:
        ...
        if self.cluster_centres is None:
            if y is None:
                ...
                self.y_pred = y
                self.cluster_centres = self.get_cluster_centers(self.encoder.predict(X), y)
    ...
    def get_cluster_centers(self, X, Y):
        freq = np.bincount(Y).reshape(-1, 1)
        sums = np.zeros((freq.shape[0], X.shape[1]))
        ...
        while train:
            ...
            if iteration % update_interval == 0:
                ...
                if iteration > 0:
                    ...
                    # when iteration is 0, self.y_pred includes actual labels for unlabeled wordnet data
                    # so we do not want to include those labels
                    delta_label = (
                                (y_pred[:cut_off] == self.y_pred[:cut_off]).sum().astype(np.float32) / y_pred[:cut_off].shape[0])

                if y is not None:

                    purity, inverse_purity, f_purity, bcubed_precision, bcubed_recall, bcubed_fscore = external_eval_clusters(y, y_pred)

                    # save model
                    if not self.bcubed_fscore or bcubed_fscore > max(self.bcubed_fscore):
                        print("Save", 'trained_SDEC_AD/SDEC_AD_bcubed_fscore_' + str(np.round(bcubed_fscore, 5)) + '.h5')
                        self.DEC.save('trained_SDEC_AD/SDEC_AD_bcubed_fscore_' + str(np.round(bcubed_fscore, 5)) + '.h5')

                    self.bcubed_fscore.append(bcubed_fscore)
                    print_external_eval_clusters(purity, inverse_purity, f_purity, bcubed_precision, bcubed_recall, bcubed_fscore)
                    # print('Iteration ' + str(iteration) + ', Accuracy ' + str(np.round(acc, 5)) +
                    #       ', NMI ' + str(normalized_mutual_info_score(y_pred, y)))
                else:
            ...
            if (index + 1) * self.batch_size > X.shape[0]:
                # calculate additional loss using only FN LUs information for supervision
                if index * self.batch_size >= cut_off:
                    add_loss = 0
                else:
                    add_loss = SDEC.add_loss(self.encoder.predict(X[index * self.batch_size:cut_off]),
                                             y[index * self.batch_size:cut_off])

                # loss function
                self.DEC.loss = SDEC.sdec_loss(add_loss=add_loss)
                ...
                if index * self.batch_size >= cut_off:
                    ...
                    add_loss = SDEC.add_loss(
                        self.encoder.predict(X[index * self.batch_size:min((index + 1) * self.batch_size, cut_off)]),
                        y[index * self.batch_size:min((index + 1) * self.batch_size, cut_off)])
            ...
            if iteration % save_interval == 0:
                ...
                self.DEC.save('DEC_fnplus_model_' + str(iteration) + '.h5')

            iteration += 1
            sys.stdout.flush()
        return

    def train_decoders(self, SDEC_trained_weights, X, layerwise_pretrain_iters=50000, finetune_iters=100000):
        # load models
        keras.objectives.custom_loss = SDEC.sdec_loss
        model = load_model(SDEC_trained_weights, custom_objects={'ClusteringLayer': ClusteringLayer,
                                                                 'loss_function': SDEC.sdec_loss(add_loss=0)})

        # freeze the encoder layers
        # 冻结模型的权重参数
        for i in range(len(self.autoencoder.layers) // 2):
            self.autoencoder.layers[i].set_weights(model.layers[0].layers[i].get_weights())
            self.layer_wise_autoencoders[i].set_weights(model.layers[0].layers[i].get_weights())
            self.autoencoder.layers[i].trainable = False

        # iters_per_epoch表示迭代次数
        # layerwise_epochs 逐层训练轮数
        ...
        print("Reconstruction Loss:", self.autoencoder.evaluate(X, X, batch_size=self.batch_size))

    def predict(self, X, SDEC_trained_weights):
        """
        :param X: LU embeddings
        :param SDEC_model_file: saved model when running the clustering (c.cluster)
        :return: clustering of the LUs embeddings
        """
        assert SDEC_trained_weights is not None
        self.DEC = load_model(SDEC_trained_weights, custom_objects={'ClusteringLayer': ClusteringLayer,
                                                                    'loss_function': SDEC.sdec_loss(add_loss=0)})
        res = self.DEC.predict(X, verbose=0)
        res = self.p_mat(res)
        return res.argmax(1)

    def reconstruction_loss(self, X, individual=False):
        if not individual:
            return self.autoencoder.evaluate(X, X, batch_size=self.batch_size)
        else:
            individual_reconstruction_loss = []
            for i in range(X.shape[0]):
                x = np.array([X[i, :]])
                individual_reconstruction_loss.append(self.autoencoder.evaluate(x, x, batch_size=self.batch_size))
            return individual_reconstruction_loss
